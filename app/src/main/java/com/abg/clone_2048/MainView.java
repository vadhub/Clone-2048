...
//        drawMenuButton(canvas, false);
    ...
    private int[] getCellRectangleIds() {
        ...
        for (int xx = 11; xx < cellRectangleIds.length; xx++)
            cellRectangleIds[xx] = R.drawable.cell_2048;
    ...
    private void getLayout(int width, int height) {
        ...
        gameOverTextSize = Math.min(
                Math.min(1000f * ((widthWithPadding - gridWidth * 2) / (paint.measureText(getResources().getString(R.string.game_over)))), textSize * 2), 1000f * ((widthWithPadding - gridWidth * 2) / (paint.measureText(getResources().getString(R.string.you_win))))
        );
